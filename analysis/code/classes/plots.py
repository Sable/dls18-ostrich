...
class Plots:
    ...
    def plot_v2(y, y_error=None,output_file=None, x_tick=None, title=None, legends=None,
                ...
                font_size=12,q2b=False, q3a=False, q2a=False, q2c=False):
        ...
        for h in range(length_rects):
            ...
            if y_error is not None:
                re = ax.bar(pos, tuple(list(y[h])), width, color=colors[h], yerr=tuple(list(y_error[h])))
            else:
                if q3a and h == 3:
                ...
                re = ax.bar(pos, tuple(y[h]), width, color=colors[h])
            rects = rects + (re[0],)
        if speedup:
        ...
        ax.set_xticks(ind)
        ax.set_xticklabels(tuple(x_tick), fontsize=14)
        plt.legend(rects, tuple(legends), loc="upper right", fontsize=18)
