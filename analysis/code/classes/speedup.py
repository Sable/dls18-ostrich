...
def histogram_plot(input_file, output_file, bench_number, artifact_number, random_colors=False,
                   name_plot="",include_name=False,error_bars=True,y_lim=None, geo_mean = True):
    # ==============================================================================
    # Parameters
    # ==============================================================================
    ...
    # legends = ["Chrome63-wasm-c","Firefox57-wasm-c","Safari11-wasm-c","C"]
    ...
    for h in range(NUMBER_ARTIFACTS - 1):
        ...
        if error_bars:
            re = ax.bar(pos, tuple(init[h]), width, color=colors[h], yerr=tuple(err[h]))
        else:
            re = ax.bar(pos, tuple(init[h]), width, color=colors[h])
        rects = rects + (re[0],)
    baseline = ax.plot(np.arange(N + 2), np.ones(N + 2), "k--", linewidth=2)
    rects = rects + (baseline[0],)
    ax.set_ylabel('Speedup (relative to C)',fontsize=18)
    if include_name:
    ...
    ax.set_xticks(ind)
    ax.set_xticklabels(tuple(xtick), fontsize=12)
    ax.legend(rects, tuple(legends), loc="upper right",fontsize=18)
